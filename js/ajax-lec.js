...
// console.log(obj.key); <--to view the obj as in lecture

// We still interact with this object just the same.


// Let's make an API call to get some AJAX going.
// <--api request below - as in lecture
// $.get('https://pokeapi.co/api/v2/pokemon/dragonite').done(function (info) {
//     console.log(info);   <-- console.log(info.held_items[0].item.url);  --> info from api call -> gets the url to request the info
//          <-- $.get(info.held_items[0].item.url).done(function(data){  <-- call within a call
//          <--- console.log(data.flavor_text_entries[6].text);
// });

// <-- Star Wars api -->
// /jquery call | .get request | url api request | .done take info received and do something with it | function(resp) and console log info.
